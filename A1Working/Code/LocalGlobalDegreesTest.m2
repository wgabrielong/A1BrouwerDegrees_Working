    ...
    if (instance(kk, PolynomialRing) and numgens(ambient(coefficientRing(kk))) == 1) then (
        ...
        try D = mutableMatrix id_((frac R)^n) else D = mutableMatrix id_(R^n);
                ...
                for j from 0 to (n-1) do(
                    ...
                    -- create the list {a, Y_1, ..., Y_(j-1), X_j, ..., X_n}. Note Macaulay2 is 0-indexed hence the difference in notation. 
                    targetList1 := apply(toList (Y_1..Y_j|X_(j+1)..X_n),i->i_R);
                    -- create the list {a, Y_1, ..., Y_j, X_(j+1), ..., X_n }. Note Macaulay2 is 0-indexed hence the difference in notation. 
                    targetList2 := apply(toList (Y_1..Y_(j+1)| X_(j+2)..X_n),i->i_R);  
                    --print prepend(a, toList (Y_1..Y_j|X_(j+1)..X_n));
                    --print prepend(a, toList (Y_1..Y_(j+1)| X_(j+2)..X_n));
                    ...
                    --print (map(R,S,targetList1))(Endo_i);
                    --print (map(R,S,targetList2))(Endo_i);
